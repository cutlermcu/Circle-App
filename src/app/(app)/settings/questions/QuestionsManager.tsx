...
export default function QuestionsManager({
  ...
  const filteredGlobal = filterCategory === 'All'
    ? globalQuestions
    : globalQuestions.filter(q => q.category === filterCategory)
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
            activeTab === 'bank' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
            activeTab === 'mine' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'
          }`}
        >
            ...
            {filteredGlobal.map(q => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
                  q.used ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-200'
                }`}
              >
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-400 mb-3">
              Your custom questions are private — only you can see them.
            </p>
          ...
          {myList.length > 0 ? (
            <div className="space-y-1.5">
              {myList.map(q => (
                <div key={q.id} className="flex items-start gap-3 px-4 py-3 bg-white rounded-xl border border-slate-200">
